    ...
    theFormula.save(function(err, result) {
        if (err) {
            ...
            if (subjects && typeof(subjects) == "string") {
                structs.Subject.findOneAndUpdate({
                    name: subjects
                }, {
                    $addToSet: {
                        formulas: ObjectId(result.id)
                    },
                }).exec(function(err, numModified) {
                    if (err) {
                        console.log(err);
                    } else {
                        console.log("Success " + numModified);
                    }
                });
            } else if (subjects) {
...
router.get('/categories', function(req, res) {
    structs.Category.find().populate('subjects').exec(function(err, result) {
        res.render('formula/displayCategories', {
            title: "Categories",
            categories: result
        });
    });
...
router.get('/subject/:name', function(req, res) {
    structs.Subject.findOne({
        name: req.params.name
    }).populate('formulas').exec(function(err, result) {
        if (err || !result) {
            console.log(err);
            res.render('Error', {
                message: "Error",
            ...
            res.render('formula/subjectListing', {
                title: result.name,
                subject: result
            });
        }
...
router.post('/update/:formulaId', userz.verify, function(req, res) {
    res.setHeader('Content-Type', 'application/json');
    structs.Formula.update({
        _id: ObjectId(req.params.formulaId)
    }, {
        $set: {
            formula: req.body.formula,
            title: req.body.title,
